...
void XTaskWrapper::SetShare(xdag_hash_t lastFieldMinerAddressAndMinNonce, xdag_hash_t minHash)
{
    if(XHash::CompareHashes(minHash, _task.minhash.data) < 0)
    {
        _shareMutex.lock();
        if(XHash::CompareHashes(minHash, _task.minhash.data) < 0)
        {
            memcpy(_task.minhash.data, minHash, sizeof(xdag_hash_t));
            memcpy(_task.lastfield.data, lastFieldMinerAddressAndMinNonce, sizeof(xdag_hash_t));
            _isShareFound = true;
        }
        _shareMutex.unlock();
    }
