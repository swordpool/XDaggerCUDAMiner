...
#define rot(x, y) (((x>>(32-y))&(0x7fffffff>>(31-y)))|(x<<y)) //
...
{
	//return;
	ul thread = (blockDim.x * blockIdx.x + threadIdx.x);
	ul nonce = startNonce + thread * ITERATIONS_COUNT;
	...
	for (uint i = 0; i < ITERATIONS_COUNT; ++i)
	{
		...
		Round(d, e, f, g, h, a, b, c, 0xa4506cebU, w13);
		///////////////////
		//we don't need to do these last 2 rounds as they update F, B, E and A, 
		//but we only care about G and H
		//w14 += sigma1(w12) + w7 + sigma0(w15);
		//Round(c, d, e, f, g, h, a, b, 0xbef9a3f7U, w14);
		//w15 += sigma1(w13) + w8 + sigma0(w0);
		//Round(b, c, d, e, f, g, h, a, 0xc67178f2U, w15);
	...
	{
		uint slot = atomicAdd((uint*)output, 1) + 1;
		slot = OUTPUT_SIZE > slot ? slot : OUTPUT_SIZE ;
		output[slot] = minNonce;
	}
}
#include <stdio.h>
__host__ void call_kernel(
	ulong startNonce,
	u* state,
	u* precalcState,
	u* data,
	...
	u throughput)
{
	...
	dim3 grid((throughput + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);
	/*
	cudaEvent_t start_device=0, stop_device=0;
	float time_device = 0;
	cudaEventCreate(&start_device);
	cudaEventCreate(&stop_device);
	cudaEventRecord(start_device, 0);
	*/
	search_nonce <<<grid, block>>>(
		startNonce,
		state,
		precalcState,
		data,
	...
	cudaError cudaStatus = cudaEventElapsedTime(&time_device, start_device, stop_device);
