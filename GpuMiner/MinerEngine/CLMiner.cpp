...
void CLMiner::WriteKernelArgs(XTaskWrapper* taskWrapper, uint64_t* zeroBuffer)
{
	...
	uint32_t targetH = ((uint32_t*)taskWrapper->GetTask()->minhash.data)[7];
	uint32_t targetG = ((uint32_t*)taskWrapper->GetTask()->minhash.data)[6];
    _searchKernel.setArg(KERNEL_ARG_TARGET_H, targetH);
    _searchKernel.setArg(KERNEL_ARG_TARGET_G, targetG);
