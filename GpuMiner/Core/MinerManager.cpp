...
bool MinerManager::InterpretOption(int& i, int argc, char** argv)
{
	...
	{
		while (_cudaDeviceCount < 16 && i + 1 < argc)
		{
			try
			{
				_cudaDevices[_cudaDeviceCount] = stoi(argv[++i]);
				++_cudaDeviceCount;
			}
			catch (...)
			{
...
void MinerManager::Execute()
{
    if(_shouldListDevices)
    {
		...
		if ((_minerType & MinerType::CUDA) == MinerType::CUDA)
		{
			CLMiner::ListDevices(_useOpenClCpu);
		}
...
void MinerManager::DoMining(MinerType type, string& remote, unsigned recheckPeriod)
{
	...
	if ((type & MinerType::CUDA) == MinerType::CUDA)
	{
		farm.AddSeeker(Farm::SeekerDescriptor{ &CUDAMiner::Instances, [](unsigned index, XTaskProcessor* taskProcessor) { return new CUDAMiner(index, taskProcessor); } });
	}
...
void MinerManager::ConfigureCuda()
{
	if (_cudaDeviceCount > 0)
	{
		CUDAMiner::SetDevices(_cudaDevices, _cudaDeviceCount);
		_cudaMiningDevices = _cudaDeviceCount;
	}
	
	if (!CUDAMiner::ConfigureCuda(
		_localWorkSize,
		_globalWorkSizeMultiplier,
		_openclPlatform,
		_useOpenClCpu))
	...
	CUDAMiner::SetNumInstances(_openclMiningDevices);
}
...
bool MinerManager::CheckMandatoryParams()
{
    return (_shouldListDevices && _minerType != MinerType::NotSet)
        ...
        || ((_minerType == MinerType::CPU || _minerType == MinerType::CL || _minerType == MinerType::CUDA ) && !_accountAddress.empty() && !_poolUrl.empty());
}
